...
st.set_page_config(page_title="Quejas", layout="wide")
# ======================================================
...
# Métrica de retrasos: Total Retrasos y % de retrasos
if "retraso" in filtered_df.columns:
    ...
    total_retrasos = 0
    porc_retrasos = 0
    num_ordenes_retrasadas = 0

# Cálculo de órdenes a tiempo
ordenes_a_tiempo = total_ordenes - num_ordenes_retrasadas
porc_tiempo = (ordenes_a_tiempo / total_ordenes * 100) if total_ordenes > 0 else 0

# Mostrar todas las métricas en una sola fila
...
col6.metric("% Órdenes a Tiempo", f"{porc_tiempo:.2f}%")
...
# ======================================================
# Sección: Retrasos por Marca
# ======================================================
st.markdown("## Total de Retrasos por Marca")

if "retraso" in filtered_df.columns and "marca" in filtered_df.columns:
    df_retrasos_marca = filtered_df[filtered_df["retraso"].notnull() & (filtered_df["retraso"] > 0)].copy()
    if not df_retrasos_marca.empty:
        tabla_retrasos = df_retrasos_marca.groupby("marca").size().reset_index(name="Total de Retrasos")
        st.dataframe(tabla_retrasos)
    else:
        st.write("No hay retrasos registrados por marca.")
else:
    st.write("No hay datos de retrasos o marca disponibles.")
